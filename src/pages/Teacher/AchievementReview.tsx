...
const AchievementReview = () => {
  ...
  const handleReview = async (achievementId: string, status: 'approved' | 'rejected') => {
    ...
    try {
      ...
      toast.error('Failed to review achievement');
    } finally {
      setReviewingId(null);
    }
  };
  ...
  return (
    <div className="p-6">
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-gray-900">Achievement Review</h1>
        <p className="text-gray-600">Review and manage student achievements</p>
      </div>
                ...
                {achievement.status === 'pending' && (
                  <div className="space-y-4">
                    ...
                    <div className="flex space-x-3">
                      <Button
                        onClick={() => handleReview(achievement.id, 'approved')}
                        loading={reviewingId === achievement.id}
                        variant="primary"
                      ...
                      <Button
                        onClick={() => handleReview(achievement.id, 'rejected')}
                        loading={reviewingId === achievement.id}
                        variant="danger"
